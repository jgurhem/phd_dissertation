...
\section{Description of the computers used}

\subsection{K Computer}
...
In the first place, the multi-level programming YML/XMP will be compared to XMP on the K computer where OmniRPC and XMP are installed and optimized.
Then, compare YML/XMP and XMP will be compared to ScaLAPACK and the implementation of the resolution of a dense linear in MPI on Poincare.


\subsection{Resolution of a linear system of size 16384 on 1024 cores}
...
The matrix is already generated and each YML component has to load its data from the file system, makes the computations on the data then saves its results to the file system.
...
We used from $1\times 1$ block to $16\times 16$ blocks with tasks from 16 to 512 cores from the 1024 cores of the K Computer allocated to each run.
...
\begin{figure}
	...
	\centering
	\includegraphics[width=.49\textwidth]{figK-g-16k.pdf}
\end{figure}

In the Figure \ref{fig:K_g}, we observe the impact of the number of blocks and the number of cores per task on the execution time.
...
In this case, XMP was faster than YML/XMP.
We think that the number of cores is too small thus the overhead from YML doesn't compensate for direct communications across all the cores.
...
\subsection{Resolution of a linear system of size 32768 and 65536 on 8096 cores}
We also made experiments on larger systems with more cores : 32768 and 65536 on 8096 cores.
The Table \ref{tab:K} summarize the execution times obtained.

\begin{table}
	\caption{Execution time (s) to solve a linear system with the Gaussian elimination on the K computer with 8096 cores\label{tab:K}}
	\centering
	\begin{tabular}{ccccc}
		\hline
		Size  &      \multicolumn{3}{c}{YML/XMP}      &  XMP  \\
     &    blocks    & cores/task & best time &       \\ \hline
		32768 & $4 \times 4$ &    512     &   276.8   & 508.5 \\
		65536 & $8 \times 8$ &    512     &    690    & 2512  \\ \hline
	\end{tabular}
\end{table}

In this experiment, we increased the size of the system and the number of cores.
In this case, YML/XMP performed better than XMP alone for the two different size whereas XMP was faster than YML/XMP on smaller cases.
...
\subsection{Evaluation and analysis on a cluster\label{sec:exp}}
...
The Table \ref{tab:best_time} gives a summary of the results obtained with the different programming paradigms.
