...
In the following algorithms, $A_{i,j}^{(k)}$ represents the $k^{th}$ step of the block at the $i^{th}$ row and $j^{th}$ column of the block matrix $A$.
...
\begin{algorithm}[h]
	...
	\For{k \KwFrom 0 \KwTo p-2}{
		(1) {\color{red} $\displaystyle Inv^{(k)} = [A_{k,k}^{(k)}]^{-1}$ } \;
		(2) {\color{magenta} $b_k^{(k+1)} = Inv^{(k)} \cdot b_k^{(k)}$ }\;
		\For{j \KwFrom k+1 \KwTo p-1}{
			(3) {\color{blue} $\displaystyle A_{k,j}^{(k+1)} = Inv^{(k)} \cdot A_{k,j}^{(k)}$ } \;
		}
		\For{i \KwFrom k+1 \KwTo p-1}{
			(4) {\color{YellowGreen} $b_i^{(k+1)} = b_i^{(k)} - A_{i,k}^{(k)} \cdot b_k^{(k+1)}$ } \;
			\For{j \KwFrom k+1 \KwTo p-1}{
				(5) {\color{OliveGreen} $A_{i,j}^{(k+1)} = A_{i,j}^{(k)} - A_{i,k}^{(k)} \cdot A_{k,j}^{(k+1)}$ } \;
			}
		}
	}
	...
	\For{k \KwFrom 1 \KwTo p-1 }{
		\For{i \KwFrom 0 \KwTo p-k-1 }{
			(7) {\color{YellowGreen} $b_i^{(k+i+1)} = b_i^{(k+i)} - A_{i,p-k}^{(i+1)} \cdot b_{p-k}^{(p)}$ } \;
		}
	}
\end{algorithm}
	...
	\For{k \KwFrom 0 \KwTo p-1}{
		...
		\For{j \KwFrom k+1 \KwTo p-1}{
			...
			\For{i \KwFrom 0 \KwTo p-1}{
				\If{k $\neq$ i}{
					(4) {\color{OliveGreen} $A_{i,j}^{(k+1)} = A_{i,j}^{(k)} - A_{i,k}^{(k)} \cdot A_{k,j}^{(k+1)}$ } \;
				}
			}
		}
		\For{i \KwFrom 0 \KwTo p-1}{
			\If{k $\neq$ i}{
				(5) {\color{YellowGreen} $b_i^{(k+1)} = b_i^{(k+1)} - A_{i,k}^{(k)} \cdot b_k^{(k+1)}$ } \;
			}
		}
	}
\end{algorithm}
...
\begin{algorithm}[h]
	...
	\For{i \KwFrom 0 \KwTo p-1}{
		\For{j \KwFrom 0 \KwTo p-1}{
			init($A_{i,j}^{(0)}$)
		}
	}
	\For{k \KwFrom 0 \KwTo p-2}{
		{\color{red} $\displaystyle Inv^{(k)} = [A_{k,k}^{(k)}]^{-1}$} \;
		\For{i \KwFrom k+1 \KwTo p-1}{
			{\color{blue} $\displaystyle A_{i,k}^{(k+1)} = A_{i,k}^{(k)} \cdot Inv^{(k)}$} \;
			\For{j \KwFrom k+1 \KwTo p-1}{
				{\color{OliveGreen} $A_{i,j}^{(k+1)} = A_{i,j}^{(k)} - A_{i,k}^{(k+1)} \cdot A_{k,j}^{(k)}$} \;
			}
		}
	}
\end{algorithm}
...
\begin{algorithm}[h]
	...
	\For{i \KwFrom 0 \KwTo p-2}{
		\For{j \KwFrom i+1 \KwTo p}{
			(5) {\color{YellowGreen} $b_j^{(i+1)} = b_j^{(i)} - A_{j,i}^{(i+1)} \cdot b_i^{(p)}$ } \;
		}
	}
	\;
	\For{k \KwFrom p-1 \KwTo 0 \KwStep -1}{
		(6) {\color{Cyan}  solve $b_{k}^{(p)} = A_{k,k}^{(k)} \cdot b_k^{(p-1)}$ } \;
		\For{i \KwFrom 1 \KwTo k-1}{
			(7) {\color{YellowGreen} $b_i^{(p-k+i)} = b_i^{(p-k+i-1)} - A_{i,k}^{(i)} \cdot b_k^{(p)}$ } \;
		}
	}
\end{algorithm}

In Figure \ref{fig:graph_sls_lu}, we added the backward and forward substitution to the LU factorization.
...
For this experiments, the Algorithm \ref{alg:bg_el_block_colo} is expressed as YvetteML graph in Listing \ref{lst:yml_graph_bg} and the operations on blocks are implemented using XMP.
...
\lstinputlisting[tabsize=4, style=customcode,
caption=YML Graph for the Gaussian elimination\label{lst:yml_graph_bg}]{chapters/exp_dense/bg.query}
...
We can outline the fact that a programming paradigm based on a graph of parallel and distributed tasks can obtain better performances on a huge amount of cores.
...
Furthermore, we implemented these applications with MPI and ScaLAPACK since MPI is the commonly used library to implement parallel and distributed applications and ScaLAPACK is an efficient implementation of the common dense linear operations.
...
Besides, the installation of the different task based programming models is not trivial.
...
%\begin{itemize}
%\item Implementation of the solution of dense linear systems with three block based methods
%\item Implementation using ScaLAPACK, MPI, XMP and YML+XMP
%\item Experiments on K with YML+XMP and XMP
%\item Experiments on Poincare with ScaLAPACK, MPI, XMP and YML+XMP
%\item Implementation of a block based LU factorization with MPI, ScaLAPACK, XMP, YML+XMP, Regent, HPX and PaRSEC
%\item Installation and set up of ScaLAPACK, XMP, YML+XMP, Regent, HPX and PaRSEC on Poincare
%\item Experiments on Poincare
%\end{itemize}
%
...
%\end{itemize}
%
%\subsection{pb rencontrés}
%\begin{itemize}
%\item Installation of Regent and YML+XMP complex
%\item The YML+XMP type that holds dense matrices does not support non divisible values and greatly limits the number of blocks that can be used.
%\item parameter (number of blocks, size of the blocks) exploration
%\end{itemize}
%
%\subsection{pb resolus}
%\begin{itemize}
%\item Successful installation of the programming models and languages used in this study
%\end{itemize}
%
%\subsection{points à mettre en avant}
%\begin{itemize}
%\item Programming models complex to install
%\item Use of file system to transmit data between tasks is not efficient enough on the current clusters and supercomputer
%\end{itemize}
%
%\subsection{ce qui est bien}
%\begin{itemize}
%\item YML+XMP faster than XMP on the K Computer
%\item YML+XMP has better strong scaling speedup than HPX, Regent and PaRSEC
%\end{itemize}
%
%\subsection{ce qui n'est pas bien}
%\begin{itemize}
%\item Use of file system to transmit data between tasks is not efficient enough on the current clusters and supercomputer
%\item Installation difficult
%\item Difficult to infer the number of blocks and size of task (parameters to determine the number of tasks executed)
%\item Regent has difficulties to scale above 32 nodes
%\item Regent default mapper is not very efficient
%\item Inefficient IOs with ScaLAPACK data distribution
%\item YML has a large overhead for each component.
%\end{itemize}
%
%\subsection{perspectives / ce qu'il reste à faire}
%\begin{itemize}
%\item Improve YML+XMP dense matrix type
%\item Compare with other programming models such as X10, Chapel, Uintah or Charm++
%\item Reproduce experiments on other clusters / supercomputers
%\item Use other type of applications such as sparse linear algebra. Next chapter --> sparse matrix vector product + experimentations with sequences of sparse matrix vector products
%\item better file systems and schedulers
%\end{itemize}
