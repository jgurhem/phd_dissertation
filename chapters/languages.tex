...
A YML+XMP application for dense linear algebra was previously implemented and was promoted in our research group so it used in this dissertation.
Swift and Pegasus were chosen for their ability to run distributed and parallel tasks which is close to what is available in YML.
We used Legion and Regent which is a simpler language made to more easily access Legion capabilities since Total is interested in Legion.
HPX is a distributed extension of the C++ standard and is closely following C++ standards.
It could become a standard way of defining and executing tasks on parallel and distributed resources.
Therefore, HPX has been selected to implement our applications.
TensorFlow is considered for its task based approach for linear algebra.
It is also a library which is increasingly used and popularx.
Finally, PaRSEC is used due to its integration into the linear algebra library DPLASMA and its dependency model based on data.
